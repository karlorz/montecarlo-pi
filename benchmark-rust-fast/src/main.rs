...
/// Calculate Pi using Monte Carlo method with fastrand (parallel version)
fn calculate_pi_parallel(iterations: u64, threads: usize, run_seed: u64) -> f64 {
    ...
    (0..threads).into_par_iter().for_each(|thread_id| {
        // Seed fastrand for this thread and run combination
...
fn benchmark(iterations: u64, runs: u32, threads: usize) {
    println!("\n=== Rust Monte Carlo Pi Benchmark (fastrand - Parallel) ===");
